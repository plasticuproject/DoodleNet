...
import numpy as np
import cv2
from dutil import auto_canny


IMAGE_DIR = 'faces'
parser = argparse.ArgumentParser(description='Data Generator')
parser.add_argument('--test', action='store_true', help='use images in test folder instead of faces')
parser.add_argument('--sample', type=int, nargs='?', help='samples per image, default is 10)', default=10)
args = parser.parse_args()

if args.test == True:
    IMAGE_DIR = 'test'

NUM_IMAGES = len(next(os.walk(IMAGE_DIR))[2])
SAMPLES_PER_IMG = args.sample
DOTS_PER_IMG = 60
IMAGE_W = 144
IMAGE_H = 192
...
y_data = np.empty((NUM_SAMPLES, NUM_CHANNELS, IMAGE_H, IMAGE_W), dtype=np.uint8)
ix = 0
    ...
    for file in files:
        ...
        for i in range(SAMPLES_PER_IMG):
            ...
            if ix < SAMPLES_PER_IMG*16:
                outimg = x_data[ix][0]
                cv2.imwrite('cargbs/cargb' + str(ix) + '.png', outimg)
                cv2.imwrite('cargbs/image' + str(ix) + '.jpg', img)
                print(path)
